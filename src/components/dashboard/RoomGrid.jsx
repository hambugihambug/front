  ...
  return (
    <div>
      <div className="floor-tabs">
        {floors.map(floor => (
          <button
            ...
            onClick={() => { setSelectedFloor(floor); setCurrentPage(0); }}
          >
            {floor}층
          </button>
        ))}
      ...
      <div className="pagination-controls" style={{ marginTop: '12px', textAlign: 'center' }}>
        <button disabled={currentPage === 0} onClick={() => setCurrentPage(p => Math.max(p - 1, 0))} className="btn-small mr-2">
          이전
        </button>
        <span>{currentPage + 1} / {totalPages}</span>
        <button disabled={currentPage >= totalPages - 1} onClick={() => setCurrentPage(p => Math.min(p + 1, totalPages - 1))} className="btn-small ml-2">
          다음
        </button>
      </div>
    </div>
